...
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/iVdFjJNtaUlTFjgyyYVV/deploy/configuration/hcm-console-configuration/console-ui-configuration/hcm-console-web-ui/manage-boundary-data
---
...
* Create: `/project-factory/v1/data/_create` The data Create API is called if it’s a new hierarchy, and here, a polling mechanism is installed, which calls the API.                                                                                                `/project-factory/v1/data/_search` continuously at a fixed interval until the status is “completed”
...
2. The search API `/project-factory/v1/data/_search` is called repeatedly with frequency depending on the size of data and network latency, taken from the mdms data using the master name `“baseTimeout”`  from a module named `HCM-ADMIN-CONSOLE`until status is “completed”.
